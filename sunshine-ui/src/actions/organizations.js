...
export function getAllOSSList(qp) {
      const config = {
        method: 'GET',
        credentials: 'include',
      ...
      return dispatch => {
        ...
        return fetch(ENDPOINTS.SERVER + `/oss/admins`, config)
          .then(response => {
            if (!response.ok) {
              throw new Error(response.statusText);
            }
            dispatch({ type: SET_ALL_OSSLIST_NUMBER, payload: response.headers.get('X-Documents-Count') });
            return response.json();
          })
          .then(data => {
            // SUCCESS -> STOP FETCHING
            dispatch({ type: ALL_OSSLIST_SUCCESS, payload: data });
          })
          .catch(() => {
            dispatch({ type: ALL_OSSLIST_FAILURE });
