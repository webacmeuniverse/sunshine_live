...
export function access(user, permission) {
  ...
  if (permission === 'admin') {
    return Boolean(user?.isSuperUser) || user?.profileInfo?.data?.platform_manager || user?.profileInfo?.data?.admin_nw_manager;
  }

  if (typeof permission === 'object') {
    if (permission.admin && (user?.isSuperUser || user?.profileInfo?.data?.platform_manager || user?.profileInfo?.data?.admin_nw_manager)) {
      return true;
    }
