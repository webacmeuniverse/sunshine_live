...
// their default values might match. This explicit setting of associations
// configuration, however, is falsy as it takes precedence over models'
// `gorm:"association_..."` annotations and makes it impossible to use. Therefore
// `.Set("gorm:association_autoupdate", true)` is removed from the list below, as
// it is already the default value, but CAN be overloaded via `gorm` model annotations.
