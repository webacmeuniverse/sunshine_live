...
func (m *meeting) getFile(w http.ResponseWriter, r *http.Request) {
	...
	if md.Project != nil {
		...
		proj = pdoc.Data.(*models.Project)
		if controller.Can(r.Context(), controller.DownloadProjectFile, proj.ID, proj.Country) {
			hasProjRole = true
		}
	}

	host := md.Host
	hasOrgRole := m.authorized(r.Context(), services.Session(m.session, r), host)

	// user has role in consortium org of the host
	hasCOrgRole := false
	for _, corg := range proj.ConsortiumOrgs {
		if m.authorized(r.Context(), services.Session(m.session, r), uuid.MustParse(corg)) {
			hasCOrgRole = true
		}
	}
	if !hasOrgRole && !hasCOrgRole && !hasProjRole {
		http.Error(w, "", http.StatusUnauthorized)
		return
	}
