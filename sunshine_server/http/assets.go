...
// List my assets. Does not preload dependencies
func (a *asset) listMyAssets(w http.ResponseWriter, r *http.Request) {
	docs, deps, n, err := a.c.ListMyAssets(r.Context(), extractUUID(r), ParseFilter(r.URL.Query()))

	w.Header().Set(countHeader, strconv.Itoa(n))
	encode(w, docs, deps, err)
}

func (a *asset) listApprovedAssets(w http.ResponseWriter, r *http.Request) {
	docs, deps, n, err := a.c.ListApprovedAssets(r.Context(), extractUUID(r), ParseFilter(r.URL.Query()))
