...
func (ch *contractHandler) downloadNativeAnnexNPDF(w http.ResponseWriter, r *http.Request) {
	ch.downloadAnnexN(w, r, "native", "pdf")
}

func (ch *contractHandler) downloadEnglishAnnexNPDF(w http.ResponseWriter, r *http.Request) {
	ch.downloadAnnexN(w, r, "english", "pdf")
}
...
func (ch *contractHandler) downloadAnnexN(w http.ResponseWriter, r *http.Request, language, format string) {
	id := mustExtractUUID(r)

	file, name, err := ch.c.DownloadAnnexN(r.Context(), id, format, language)
	if err != nil {
		writeError(w, r, err)
		return
	}
