...
func testPrjUploadEscapeSymbols(t *testing.T) {
	...
	f, err := os.ReadFile(`testdata/g g.jpg`)
	if err != nil {
		t.Fatal(err)
	}
...
func testProjectCreate(t *testing.T) {
	var (
		...
		org         = stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(user.ID), stores.TOwithLEAA(invalid.ID))
...
func testInvalidOrg(t *testing.T) {
	...
	org := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(user.ID))
	asset := stores.NewTestAsset(t, e.AssetStore, stores.TAWithOrg(org.ID))
	...
	var (
		...
		cases = []struct {
		...
		}{
			{
				...
				user:     admin.ID,
				code:     http.StatusOK,
				prjRoles: map[string][]uuid.UUID{"pm": {pm.ID}},
			...
			},
			{
				...
				prjRoles: map[string][]uuid.UUID{"teme": {teme.ID}},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.code == http.StatusOK {
				...
				epcs := doc.Data.(*models.Project).EPCSigned
				if !epcs {
					t.Fatal("expected project to be signed")
				}

				fas := doc.Data.(*models.Project).EPCSigned
				if !fas {
					t.Fatal("expected project fa to be signed")
				}
			}
...
func testProjectUpdatePD(t *testing.T) {
	...
	var (
		...
		_ = stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(dir.ID))
...
func testProjectList(t *testing.T) {
	...
	stores.NewTestProject(t, e.ProjectStore, stores.TPrjWithMilestone(models.MilestoneKickOffMeeting))
	pWp := stores.NewTestProject(t, e.ProjectStore)
	pMp := stores.NewTestProject(t, e.ProjectStore)
	pMp2 := stores.NewTestProject(t, e.ProjectStore)
	_, err := wp.AdvanceToWorkPhase(ctx, pWp.ID)
	if err != nil {
		t.Error("failed to advance to workphase")
	}
	_, err = mp.AdvanceToMonitoringPhase(ctx, pMp.ID)
	if err != nil {
		t.Error("failed to advance to monitoring")
	}
	_, err = mp.AdvanceToMonitoringPhase(ctx, pMp2.ID)
	if err != nil {
		t.Error("failed to advance to monitoring")
	}
	...
	cases := []struct {
		...
		{
			name: "milestone only",
			path: "/project?state=kick_off_meeting",
		},
		{
			name: "monitoringPhase",
			path: "/project?state=monitoring_phase",
		},
		{
			name: "workPhase",
			path: "/project?state=work_phase",
		},
		{
			name: "all reports",
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.name == "milestone only" && count != 1 {
				t.Errorf("failed to filter by milestone expected 1 got %v results", count)
			}
			if c.name == "monitoringPhase" && count != 2 {
				t.Errorf("failed to filter by monitoringPhase expected 2 got %v results", count)
			}
			if c.name == "workPhase" && count != 1 {
				t.Errorf("failed to filter by workPhase expected 1 got %v results", count)
			}
		})
	}
}
