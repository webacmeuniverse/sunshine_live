...
func (p pstore) GetPortfolioNoRole(ctx context.Context, country models.Country) (uuid.UUID, error) {
	...
	if err != nil || usr.UserID == uuid.Nil {
		err = p.db.Table("country_roles").
			...
			First(&usr).Error
		if err != nil {
			return uuid.Nil, fmt.Errorf("no such user in %q: %v", country, err)
		}
	}
...
func (p pstore) GetAllAdminNWManagers(ctx context.Context) ([]uuid.UUID, error) {
	var userid []uuid.UUID
	return userid, p.db.
		Table("users").
		Select("id").
		Where("admin_network_manager = ? ", true).
