...
func (wp *WorkPhase) AdvanceToWorkPhase(ctx context.Context, pID uuid.UUID) (*models.Document, error) {
	...
	for _, pr := range proj.ProjectRoles {
		n := models.Notification{
			...
			Action:      models.UserActionAdvanceToWorkPhase,
			UserID:      cv.User.ID,
...
func (mp *MonitoringPhase) AdvanceToMonitoringPhase(ctx context.Context, pID uuid.UUID) (*models.Document, error) {
	...
	cv := services.FromContext(ctx)
	//notify all users with project role
	for _, pr := range prj.ProjectRoles {
		n := models.Notification{
			RecipientID: pr.UserID,
			Action:      models.UserActionAdvanceToMonitoringPhase,
			UserID:      cv.User.ID,
			UserKey:     cv.User.Name,
			TargetID:    prj.ID,
			TargetKey:   prj.Name,
