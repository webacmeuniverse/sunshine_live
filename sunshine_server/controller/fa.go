...
func (f *ForfaitingAgreement) Create(ctx context.Context, fa models.ForfaitingApplication) (*models.ForfaitingApplication, error) {
	...
	fa.Reviews = []models.FAReview{
		...
		{Approved: false, Type: models.FAReviewTypeEPC},
	}

	admins, err := f.pf.GetPortfolioRolesPerCountry(ctx, prj.Country, models.FundManagerRole)
	if err == nil {
		anms, err := f.pf.GetAllAdminNWManagers(ctx)
		if err == nil {
			...
			notifyAll(ctx, f.notifier, admins, n)
		} else {
			sentry.Report(err)
		}
	} else {
	...
	// notify the new fa manger
	go f.notifier.Notify(ctx, &notfication)
...
func (f *ForfaitingAgreement) can(ctx context.Context, a Action, prjID uuid.UUID) bool {
	...
	prj := prjd.Data.(*models.Project)
	ids := []uuid.UUID{prjID, prj.Owner}
	for _, id := range prj.ConsortiumOrgs {
		ids = append(ids, uuid.MustParse(id))
	}

	return canGetProject(ctx, a, prj.Country, ids...)
}
...
func (f *ForfaitingAgreement) Update(ctx context.Context, faID uuid.UUID, fa models.ForfaitingApplication) (*models.ForfaitingApplication, error) {
	...
	if fa.ManagerID != uuid.Nil {
		...
		// in such a case
		if err == nil {
			oldManagerID = foundFA.ManagerID
	...
	if oldManagerID != fa.ManagerID {
		// check for new manager assigned
		// notify the new Fa manager being assigned forfaiting contract person role
		_, err := f.st.FromKind("user").Get(ctx, foundFA.ManagerID)
		if err != nil {
			tx.Rollback()
			return nil, err
		}
		pdoc, err := f.st.FromKind("project").Get(ctx, foundFA.Project)
		if err != nil {
			tx.Rollback()
			return nil, err
		}
		...
		err = f.notifier.Notify(ctx, &notfication)
		if err != nil {
			fmt.Println(err)
			return nil, err
		}
...
func (f *ForfaitingAgreement) GetFile(ctx context.Context, faid uuid.UUID, filename string) (*models.Attachment, *os.File, error) {
	...
	ids := []uuid.UUID{pdoc.ID, prj.Owner}
	for _, id := range prj.ConsortiumOrgs {
		ids = append(ids, uuid.MustParse(id))
	}

	if !canGetProject(ctx, GetFAAttachment, prj.Country, ids...) {
