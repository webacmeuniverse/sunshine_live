...
func TestCan(t *testing.T) {
	...
	org := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(user.ID), stores.TOwithLEAA(leaa.ID))
	proj := stores.NewTestProject(t, e.ProjectStore, stores.TPrjWithPm(user.ID))

	// The PD must be valid member of any org
	stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(pdirector.ID))
