...
func TestUpdateValidateAsset(t *testing.T) {
	...
	o := stores.NewTestOrg(t, a.store.FromKind("organization"), stores.TOwithLEAR(lear.ID))
...
func TestValidateAsset(t *testing.T) {
	...
	org := stores.NewTestOrg(t, a.store.FromKind("organization"), stores.TOwithLEAR(lear.ID))
...
func TestListReports(t *testing.T) {
	...
	org := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(lear.ID))
...
func TestListUserAssets(t *testing.T) {
	...
	tl := 4
	assets := makeTestAssets(t, e.AssetStore, tl, org.ID)

	stores.NewTestProject(t, e.ProjectStore, stores.TPrjWithAsset(assets[0].ID))
...
func TestListMyAssets(t *testing.T) {
	e := services.NewTestEnv(t)
	contr := NewAsset(e)
	...
	lear := stores.NewTestUser(t, e.UserStore)
	leas := stores.NewTestUser(t, e.UserStore)

	org := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(lear.ID), stores.TOwithLEAA(leas.ID), stores.TOwithMember(u.ID))
	ownerorg := stores.NewTestOrg(t, e.OrganizationStore)

	stores.NewTestAsset(t, e.AssetStore, stores.TAWithOrg(ownerorg.ID), stores.TAWithEsco(&org.ID))
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			_, _, total, err := contr.ListMyAssets(c.ctx, c.uid, c.filter)
			if err != c.err {
				t.Errorf("exp err: %v, but got: %v", c.err, err)
			}

			if total != c.exp {
				t.Fatalf("expected docs: %d, but got: %d", c.exp, total)
			}
		})
	}
