...
func (o *Organization) Create(ctx context.Context, r io.Reader) (*models.Document, error) {
	...
	if cv.User != nil {
		...
		if err == nil {
			anms, err := o.pf.GetAllAdminNWManagers(ctx)
			if err == nil {
...
func (o *Organization) AddRole(ctx context.Context, id uuid.UUID, body io.Reader) (*models.Document, stores.Dependencies, error) {
	...
	if err != nil {
		...
		ua := models.UserActionAssign
		if org.LegalForm == models.LegalFormResidentsCommunity {
			ua = models.UserActionAssignResidency
		}
		go o.notifier.Broadcast(ctx, ua,
...
func (o *Organization) RemoveOss(ctx context.Context, org *models.Organization) error {
	usr, err := o.FindLear(org.Roles.LEAR)
	if err != nil {
		return err
	}
	usr.IsOssAdmin = false
	org.OssApproved = false
	if err := o.store.DB().Save(&usr).Error; err != nil {
		return err
	}
	if _, err := o.store.Update(ctx, models.Wrap(org)); err != nil {
		return err
	}
	return nil
...
func (o *Organization) FindLear(uid uuid.UUID) (*models.User, error) {
	var usr models.User
	if err := o.uc.st.DB().Where("id = ?", uid).First(&usr).Error; err != nil {
		return nil, err
	}
	return &usr, nil
}

func (o *Organization) ApproveOSS(ctx context.Context, org *models.Organization) error {
	// fmt.Printf("Organization is Type OSS: %v\n", org.LegalForm)
	// fmt.Printf("Approving user of ID: %v\n", org.Roles.LEAR)
	org.OssApproved = true
	usr, err := o.FindLear(org.Roles.LEAR)
	if err != nil {
		return err
	}
	// println("user found")
	usr.IsOssAdmin = true
	// println("ready to update user")

	return o.store.DB().Save(&usr).Error
}
...
func (o *Organization) Validate(ctx context.Context, org uuid.UUID, status models.ValidationStatus, comment *string) error {
	...
	fmt.Printf("Org Validation Status: %v\n", status)
	...
	var legalForm models.LegalForm = or.LegalForm
	if legalForm == 12 {
		if err := o.ApproveOSS(ctx, or); err != nil {
			return err
		}
		if status == models.ValidationStatusDeclined {
			if err := o.RemoveOss(ctx, or); err != nil {
				return err
			}
		}
	}
	...
	// var u models.User
	// if err := o.store.DB().Where("id = ?", or.Roles.LEAR).First(&u).Error; err != nil {
	// 	return err
	// }
	if err == nil {
		// u := services.FromContext(ctx).User
...
func (o *Organization) OrgFromLear(ctx context.Context, ossAdminID string) (*models.Document, error) {
	var (
		orgRole models.OrganizationRole
	)

	doc, _, err := o.uc.GetByEmail(ctx, ossAdminID)
	if err != nil {
		return nil, err
	}

	if err := o.store.DB().Where("user_id = ?", doc.ID).First(&orgRole).Error; err != nil {
		return nil, err
	}

	doc1, _, err1 := o.Get(ctx, orgRole.OrganizationID)

	return doc1, err1
}

func (o *Organization) GetLegalForm(num int8) models.LegalForm {
	switch num {
	case 1:
		return models.LegalFormFinancialInstitution
	case 2:
		return models.LegalFormServiceCompany
	case 3:
		return models.LegalFormPublicOrganization
	case 4:
		return models.LegalFormResidentsCommunity
	case 5:
		return models.LegalFormNGO
	case 6:
		return models.LegalFormNaturalPerson
	case 7:
		return models.LegalFormSuplier
	case 8:
		return models.LegalFormHousingAssociation
	case 9:
		return models.LegalFormProfessional
	case 10:
		return models.LegalFormCraftsman
	case 11:
		return models.LegalFormOperator
	case 12:
		return models.LegalFormOSS
	default:
		return models.LegalFormFinancialInstitution
	}
}

func (o *Organization) RemoveDuplicateOrg(orgSlice []models.Organization) []models.Organization {
	allKeys := make(map[uuid.UUID]bool)
	list := []models.Organization{}
	for _, org := range orgSlice {
		if _, value := allKeys[org.ID]; !value {
			allKeys[org.ID] = true
...
func (o *Organization) FindMyPartner(ctx context.Context, rc io.Reader) ([]models.Organization, error) {
	// Type of Organization (Professional,Craftsman,Marketoperator)
	// Types of Services (Energy Audits,Technical Inspections,Project Design,Construction,Other)
	// OSS Approved (true or false)
	// country ???
	var (
		orgs    []models.Organization
		results []models.Organization
		getReq  PartnerRequest
		types   []string
	)
	...
	types = strings.Split(getReq.TypeOfServices, ",")
	if getReq.OrganizationType == 0 {
		return nil, errors.New("Bad organization type: 0")
	}

	if getReq.OrganizationType == -1 {
		if err := o.store.DB().Where("oss_approved = ?", getReq.OssApproved).Where("status = 2").Find(&orgs).Error; err != nil {
			return nil, err
		}

		for _, org := range orgs {
			for _, _type := range types {
				_typ := strings.Join(strings.Fields(_type), " ")
				if strings.Contains(org.ServicesProvided, _typ) {
					fmt.Printf("@@@ %v contained in: %v\n", _typ, org.ServicesProvided)
					results = append(results, org)
					break
				}
			}
		}
		noDups := o.RemoveDuplicateOrg(results)

		return noDups, nil
	}

	if err := o.store.DB().Where("legal_form = ?", getReq.OrganizationType).Where("oss_approved = ?", getReq.OssApproved).Where("status = 2").Find(&orgs).Error; err != nil {
		return nil, err
	}
	for _, org := range orgs {
		for _, _type := range types {
