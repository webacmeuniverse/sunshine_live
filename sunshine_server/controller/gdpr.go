...
func (g GDPR) SendRequest(ctx context.Context, req *models.GDPRRequest, u []Upload) error {
	...
	var anm models.User
	err = g.st.DB().Where("admin_network_manager = ?", true).First(&anm).Error
	if err != nil {
		return err
	}
	...
	// find all dpos of the country
	dpos, err := g.pf.GetPortfolioRolesPerCountry(ctx, usr.Country, models.DataProtectionOfficerRole)
	if err != nil {
		return err
	}
	...
	if len(dpos) == 0 {
		// when the country doesnt have any DPOS,
		// send the notification to the anm
		n := models.Notification{
			RecipientID: anm.ID,
			Action:      models.UserActionGDPR,
			UserID:      uuid.Nil,
