...
func createFA(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if len(fares.Reviews) != 5 {
				t.Errorf("expected 5 reviews, but got: %d", len(fares.Reviews))
			}
			...
			//find notification for ne forfaiting manager
			notFound := u.st.DB().
				Where("recipient = ? AND action = ? ", fares.ManagerID, models.UserActionAssign).
				First(&models.Notification{}).RecordNotFound()
			if notFound == true {
				t.Errorf(" the forfaitin manager is missing notification for tama role")
			}
		})
	}
}
