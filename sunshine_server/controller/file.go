...
func uploadFile(ctx context.Context, st stores.Store, n stores.Notifier, form RequestForm, target *models.Document, uploadPath string) error {
	...
	att, err := writeFile(uploadPath, form, target.ID)
	if err != nil {
...
func uploadPdfFile(ctx context.Context, st stores.Store, form RequestForm, fileID uuid.UUID, uploadPath string) error {
	// target represents the document type (user, asset or project)

	// form.FileHeader.Filename = generateFilename(form.FileHeader.Filename, target.Attachments)
	form.FileHeader.Filename = fileID.String()

	if !CheckFilenameRe.MatchString(form.FileHeader.Filename) {
		return ErrBadInput
	}

	att, err := writeFile(uploadPath, form, fileID)
	if err != nil {
		return fmt.Errorf("%w: %v", err, ErrFatal)
	}
	att.Comment = form.Comment
...
func notify(ctx context.Context, n stores.Notifier, form RequestForm, target *models.Document) error {
	...
	if form.Kind == "learApply" {
		...
		if cv.User.ID != o.Roles.LEAR {
			go n.Notify(ctx, &not)
		} else {
			go n.NotifyCountryRole(ctx, &not, o.Country, models.CountryAdminRole)
		}
		return nil
	}
	...
	if form.Kind == "claimResidency" {
		...
		// Try to notify the Residents' Community organization first.
		var doc *models.Document
		if asset.ESCO != nil {
			doc, err = n.GetDocument(ctx, *asset.ESCO, models.OrganizationT)
			if err != nil {
				if err != ErrNotFound {
					return err
				}
			...
			// If no such organization exists - notify the asset.Owner LEAR.
...
func writeFile(path string, form RequestForm, id uuid.UUID) (*models.Attachment, error) {
	...
	var (
		...
		fh         *multipart.FileHeader = form.FileHeader
	...
	println("File name:", form.FileHeader.Filename)
	if kind == "cert" {
		filePath = filepath.Join(path, form.FileHeader.Filename)
	} else {
		filePath = filepath.Join(path, fmt.Sprintf("%s-%s", id, att.Value.ID))
	}
