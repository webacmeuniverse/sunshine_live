...
func (p *Project) Create(ctx context.Context, r io.Reader) (*models.Document, stores.Dependencies, error) {
	...
	proj.PortfolioDirector, err = p.pf.GetPortfolioNoRole(ctx, asset.Country)
	if err != nil {
		return nil, nil, err
	}

	if asset.Owner != proj.Owner {
		proj.ConsortiumOrgs = append(proj.ConsortiumOrgs, asset.Owner.String())
	}
	if asset.ESCO != nil {
		proj.ConsortiumOrgs = append(proj.ConsortiumOrgs, asset.ESCO.String())
	}
	...
	// notify asset participants all lears of (project owner, esco) organizations with projCreateOrg
	type orgLears struct {
		Name   string
		UserID uuid.UUID
	}
	lears := []orgLears{}
	orgs := make([]uuid.UUID, 0)
	orgs = append(orgs, proj.Owner)
	orgs = append(orgs, asset.Owner)
	for _, c := range proj.ConsortiumOrgs {
		cID, _ := uuid.Parse(c)
		orgs = append(orgs, cID)
	}

	p.st.DB().
		Table("organization_roles").
		Select("organizations.name,user_id").
		Joins("join organizations ON organization_id=organizations.id").
		Where("position =?", "lear").
		Where("organization_id IN (?)", orgs).
		Scan(&lears)

	for _, l := range lears {
		if l.UserID == cv.User.ID {
			continue
		}
		n := models.Notification{
			RecipientID: l.UserID,
			Action:      models.UserActionProjectCreateOrg,
			UserID:      cv.User.ID,
		...
		err = p.notifier.Notify(ctx, &n)
		if err != nil {
			return nil, nil, err
		}
	}

	// notify  portfolio directors and fund managers with pfjCreateAsset
	// gett all pd and fm with proj.Country
	cmans := make([]uuid.UUID, 0)
	p.st.DB().
		Table("country_roles").
		// removed check for user.role for adding new project here
		// Where("role IN (?)", []models.PortfolioRole{models.PortfolioDirectorRole, models.FundManagerRole}).
		Where("country = ?", asset.Country).
		Pluck("user_id", &cmans)

	n := models.Notification{
		RecipientID: uuid.UUID{},
		Action:      models.UserActionProjectCreateAsset,
		UserID:      cv.User.ID,
		UserKey:     cv.User.Name,
		TargetID:    proj.ID,
		TargetKey:   proj.Name,
	...
	err = p.notifier.NotifyMultiple(ctx, cv.User.ID, n, cmans)
	if err != nil {
		return nil, nil, err
	}
...
// canGetProject is used to check READ permissions for all members somehow affiliated with a given project
// and all its parts - contract, FA, Work/Monitoring phase, IndoorClima. These users are:
// 1. Users with direct roles in the Project
// 2. Users with a role in the Organization owner of the Project
// 3. Users with a role in the Organization owner of the Asset that the Project is registered to
// 4. Users with a role in a Organization from the Organizations Consortium related to the Project
...
func (p *Project) GetFile(ctx context.Context, pid uuid.UUID, filename string) (*models.Attachment, *os.File, error) {
	...
	prj := doc.Data.(*models.Project)
	ids := []uuid.UUID{pid, prj.Owner}
	for _, id := range prj.ConsortiumOrgs {
		ids = append(ids, uuid.MustParse(id))
	}

	if !canGetProject(ctx, DownloadProjectFile, prj.Country, ids...) {
		return nil, nil, ErrUnauthorized
	}
...
func (p *Project) AdvanceToMilestone(ctx context.Context, id uuid.UUID, m models.Milestone) error {
	...
	//notify all users with project role
	targets := make([]uuid.UUID, 0)
	for _, pr := range proj.ProjectRoles {
		targets = append(targets, pr.UserID)
	}

	n := models.Notification{
		RecipientID: uuid.UUID{},
		Action:      models.UserActionAdvanceMilestone,
		UserID:      cv.User.ID,
		UserKey:     cv.User.Name,
		TargetID:    proj.ID,
		TargetKey:   proj.Name,
		TargetType:  models.ProjectT,
		Old:         strcase.ToScreamingSnake(string(om)),
		New:         strcase.ToScreamingSnake(string(m)),
		Country:     proj.Country,
	}
	err = p.notifier.NotifyMultiple(ctx, cv.User.ID, n, targets)
	if err != nil {
		return err
	}
	return nil
