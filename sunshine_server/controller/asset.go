...
func (a *Asset) Update(ctx context.Context, id uuid.UUID, r io.Reader) (*models.Document, stores.Dependencies, error) {
	...
	if stores.ShouldInvalidate(old, updated, services.FromContext(ctx).User.SuperUser, oldValid) {
		...
		if err == nil {
			anms, err := a.pf.GetAllAdminNWManagers(ctx)
			if err == nil {
...
func (a *Asset) List(ctx context.Context, id uuid.UUID, filter stores.Filter) ([]models.Document, stores.Dependencies, int, error) {
	...
	if id != uuid.Nil {
		...
		//get all projects that user is member of.
		projs, _, _, perr := a.store.FromKind("project").ListByMember(
			ctx, stores.Filter{}, id)
		if perr != nil {
			return nil, nil, 0, fmt.Errorf("%w: %v", ErrBadInput, err)
		}
		...
		docs, deps, n, err = a.store.ListByMember(ctx, filter, IDs...)
	} else {
...
func (a *Asset) ListMyAssets(ctx context.Context, id uuid.UUID, filter stores.Filter) ([]models.Document, stores.Dependencies, int, error) {
	set := []models.Asset{}
	...
	err := a.store.DB().
		...
		Joins("inner join assets on assets.esco_id = organization_roles.organization_id").
		Where("organization_roles.user_id = ?", id).
		Where("organization_roles.position = 'member'").
		Find(&set).Error
	...
	docs := make([]models.Document, len(set))
	...
	return docs, nil, len(docs), err
}

func (a *Asset) ListApprovedAssets(ctx context.Context, id uuid.UUID, filter stores.Filter) ([]models.Document, stores.Dependencies, int, error) {
	cv := services.FromContext(ctx)
	if !cv.Authorized() {
	...
	err := a.store.DB().
		Select("assets.*").
		Table("create_project_request").
		Joins("inner join assets on assets.id = create_project_request.asset_id").
		Where("create_project_request.status='accepted'").
		Where("create_project_request.user_id= ?", cv.User.ID).
		Group("assets.id").
		Find(&result).Error
