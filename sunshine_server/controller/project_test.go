...
func TestAssignProjectRoles(t *testing.T) {
	...
	o := stores.NewTestOrg(t, p.st.FromKind("organization"), stores.TOwithLEAR(lear.ID))
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			p := pdoc.Data.(*models.Project)
			upCount := len(p.Roles.PM)
			if upCount != c.count {
				t.Errorf("expected: %v, got: %v roles", c.count, upCount)
			}
			for _, pr := range p.ProjectRoles {
				if pr.Project.ID != p.ID || pr.Project.Name != p.Name {
					t.Errorf(
						"expected id: %v and name: %v, got: id: %v and name: %v project role",
						p.ID, p.Name, pr.Project.ID, pr.Project.Name,
					)
				}
			}
		})
...
func testProcessPrjRequest(t *testing.T) {
	...
	u := stores.NewTestUser(t, e.UserStore)
	org := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(u.ID))
...
func testCreate(t *testing.T) {
	...
	org := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(orgu.ID))
	asset := stores.NewTestAsset(t, e.AssetStore, stores.TAWithOrg(org.ID))

	// guest organization
	guestu := stores.NewTestUser(t, e.UserStore)
	guesto := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(guestu.ID))
...
func testReports(t *testing.T) {
	...
	consr := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(consru.ID))

	escou := stores.NewTestUser(t, e.OrganizationStore)
	esco := stores.NewTestOrg(t, e.OrganizationStore, stores.TOwithLEAR(escou.ID))
