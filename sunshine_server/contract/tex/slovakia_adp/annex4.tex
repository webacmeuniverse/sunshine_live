...
Number of the degree days is established using the following formulas:
\begin{center}
	GDD = $D_apk$x($T_1$ - $T_3$)
\end{center}
...
where:
\begin{itemize}
	\item GDD the number of degree days during the Baseline Period
	\item $D_apk$number of the heating days during the Baseline Period
	\item $T_1$ indoor temperature during the Baseline Period  (°C).
	\item $T_3$ actual average outdoor temperature during the Baseline Period (°C).
\end{itemize}
