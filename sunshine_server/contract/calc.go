...
// PrjReportFields holds fields needed for reports that should
// update a project after doing contract calculations.
type PrjReportFields struct {
	Investment float64
	ESavings   float64
}

// Calculate contract's dynamic data.
func calculate(contr *Contract, project models.Project) (calc *Calculation, prjF *PrjReportFields, err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	// guaranteed savings
	calc.QIetG = (calc.QApkCzRef[3] * float64(project.GuaranteedSavings)) / 100
	prjF.ESavings = calc.QIetG
	...
	if err != nil {
		return nil, nil, fmt.Errorf("OM1: %w", err)
	}
	...
	prjF.Investment, _ = prjDevRenovationSum.Round(2).Float64()
	ccrs, _ := constrCostsRenovationSum.Round(2).Float64()
	prjF.Investment += ccrs
	prjspsum, _ := prjsupervisionSum.Round(2).Float64()
	prjF.Investment += prjspsum
	fchsum, _ := finchargSum.Round(2).Float64()
	prjF.Investment += fchsum

	prjF.Investment += (prjF.Investment * contr.VAT) * 0.01
