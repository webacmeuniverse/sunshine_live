...
var supportedCountries = []models.Country{
	models.CountryAustria, models.CountryBulgaria, models.CountryLatvia, models.CountryPoland, models.CountryRomania,
	...
	models.CountryGeorgia, models.CountryGreece, models.CountryHungary, models.CountryIreland, models.CountryKosovo,
	...
	models.CountryMontenegro, models.CountryNetherlands, models.CountryMacedonia, models.CountryNorway, models.CountryRussia,
	...
	models.CountryGermany, models.CountryItaly, models.CountryPortugal,
}

// models.CountryPortugal,models.CountryGermany,models.CountryItaly,
...
func (p *document) GeneratePDF(ctx context.Context, rootFile string) (*FileInTempDir, error) {
	...
	println("done generating pdf for:", genPDF.output)
...
func (p *document) generate(ctx context.Context, op texOp, root string) (*FileInTempDir, error) {
	...
	pdfDir, err = os.MkdirTemp(os.TempDir(), "sunshine_latex")
	if err != nil {
		return nil, err
	}
	if err = os.Symlink(imagesPath(), path.Join(pdfDir, "images")); err != nil {
		return nil, err
	}
	...
	go func() {
		...
		if err := md2tex(ctx, mdw, p.ctx.Contract.Markdown); err != nil {
			println("failed converting markdown :(")
	...
	return OpenFileInTempDir(filepath.Join(pdfDir, op.output))
}
...
var fmap = template.FuncMap{
	...
	"average": func(t Table) string {
		columns := make([]string, t.ColumnLen())
	...
	"period": func(lang string, t Table) Table {
		return period(lang, t)
	},
}
...
func period(lang string, t Table) Table {
	// columns
	cols := make([]Column, t.ColumnLen())
	for i, col := range t.Columns() {
		// Column Name
		colName := unmarshalPeriod(lang, col.Name)
		col.Name = colName

		// Column headers
		headers := make([]string, len(col.Headers))
		for i, h := range col.Headers {
			headers[i] = unmarshalPeriod(lang, h)
		}
		col.Headers = headers
	...
	for _, row := range t.Rows() {
		r := make([]Cell, len(row))
		for i, cell := range row {
			cellValue := unmarshalPeriod(lang, string(cell))
			r[i] = Cell(cellValue)
		}
		rows = append(rows, r)
	}
...
func unmarshalPeriod(lang string, target string) string {
	m, _ := regexp.MatchString(`\d-[ymspwa]{1,3}`, target)
	if !m {
		return target
	}
	...
	case 1:
		switch t {
		case "m":
			t = unmarshalLang(lang, monthMap)
		case "y":
			return unmarshalLang(lang, annualMap)
		case "ys":
			return unmarshalLang(lang, annualSummerMap)
		case "ya":
			return unmarshalLang(lang, annualAutumnMap)
		case "yw":
			return unmarshalLang(lang, annualWinterMap)
		case "ysp":
			return unmarshalLang(lang, annualSpringMap)
		}
	default:
		switch t {
		case "m":
			t = unmarshalLang(lang, monthsMap)
		case "y":
			t = unmarshalLang(lang, yearsMap)
		case "ys":
			return unmarshalLang(lang, strings.Replace(annualSummerCountMap, "{{count}}", d, -1))
		case "ya":
			return unmarshalLang(lang, strings.Replace(annualAutumnCountMap, "{{count}}", d, -1))
		case "yw":
			return unmarshalLang(lang, strings.Replace(annualWinterCountMap, "{{count}}", d, -1))
		case "ysp":
			return unmarshalLang(lang, strings.Replace(annualSpringCountMap, "{{count}}", d, -1))
		}
	}
...
var (
	annualSummerMap = `{"en": "Annual in Summer", "pl": "Rocznie latem", "ro": "Anual vara", "au": "Jährlich im Sommer","lv": "Katru gadu vasarā", "bg": "Ежегодно през лятото"}`
	annualAutumnMap = `{"en": "Annual in Autumn", "pl": "Rocznie jesienią", "ro": "Anual toamna", "au": "Jährlich im Herbst","lv": "Katru gadu rudenī", "bg": "Ежегодно през есента"}`
	annualWinterMap = `{"en": "Annual in Winter", "pl": "Rocznie na zimą", "ro": "Anual iarna", "au": "Jährlich im Winter","lv": "Katru gadu ziemā", "bg": "Ежегодно през зимата"}`
	annualSpringMap = `{"en": "Annual in Spring", "pl": "Rocznie na wiosnę", "ro": "Anual primăvara", "au": "Jährlich im Frühjahr","lv": "Katru gadu pavasarī", "bg": "Ежегодно през пролетта"}`
)

var (
	annualSummerCountMap = `{"en": "Every {{count}} years in Summer", "pl": "Co {{count}} lata latem", "ro": "La fiecare {{count}} ani vara", "au": "Alle {{count}} Jahre im Sommer","lv": "Vasarā reizi {{count}} gados", "bg": "Всеки {{count}} години през лятото"}`
	annualAutumnCountMap = `{"en": "Every {{count}} years in Autumn", "pl": "Co {{count}} lata jesienią", "ro": "La fiecare {{count}} ani toamna", "au": "Alle {{count}} Jahre im Herbst","lv": "Rudenī reizi {{count}} gados", "bg": "Всеки {{count}} години през есента"}`
	annualWinterCountMap = `{"en": "Every {{count}} years in Winter", "pl": "Co {{count}} lata na zimą", "ro": "La fiecare {{count}} ani iarna", "au": "Alle {{count}} Jahre im Winter","lv": "Ziemā reizi {{count}} gados", "bg": "Всеки {{count}} години през зимата"}`
	annualSpringCountMap = `{"en": "Every {{count}} years in Spring", "pl": "Co {{count}} lata na wiosnę", "ro": "La fiecare {{count}} ani primăvara", "au": "Alle {{count}} Jahre im Frühjahr","lv": "Pavasarī reizi {{count}} gados", "bg": "Всеки {{count}} години през пролетта"}`
)

var (
	monthMap  = `{"en": "month", "pl": "miesiąc", "ro": "lună", "au": "Monat",	"lv": "mēnesis", "bg": "месец"}`
	monthsMap = `{"en": "months", "pl": "miesięcy", "ro": "luni", "au": "monet","lv": "mēnesī",	"bg": "месеца"}`
	annualMap = `{"en": "Annual", "pl": "Coroczny", "ro": "Anual", "au": "Jährlich", "lv": "Gada", "bg": "Ежегодно"}`
	yearsMap  = `{"en": "years", "pl": "lat", "ro": "ani", "au": "Jahre", "lv": "gadiem", "bg": "години"}`
)
