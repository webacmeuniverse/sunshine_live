...
func (c *Contract) BeforeSave(tx *gorm.DB) (err error) {
	...
	c.VAT = GetVat(tx, proj.Country)
	// recalculate the tables with the new values and update some project fields.
	calc, prjf, err := calculate(c, proj)
	if err != nil {
		return err
	}

	// update project total investment
	proj.TotalInvestment = prjf.Investment
	proj.EnergySavings = prjf.ESavings
