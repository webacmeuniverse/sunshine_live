...
func testGet(t *testing.T) {
	...
	org := stores.NewTestOrg(t, env.OrganizationStore, stores.TOwithLEAR(user.ID), stores.TOwithLEAA(user.ID))
...
func testCreate(t *testing.T) {
	...
	admin := stores.NewTestAdmin(t, env.UserStore)
	user := stores.NewTestUser(t, env.UserStore)
	uorg := stores.NewTestOrg(t, env.OrganizationStore, stores.TOwithLEAR(user.ID))
...
func testList(t *testing.T) {
	...
	var (
		...
		_ = stores.NewTestAttachment(t, env.MeetingsStore, m1.ID, "commitment protocol meeting")
		_ = stores.NewTestAttachment(t, env.MeetingsStore, m2.ID, "pre epc agreement")
